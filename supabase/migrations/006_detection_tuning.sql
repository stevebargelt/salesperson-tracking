-- Tuning visit detection thresholds and time window
-- Apply this migration in Supabase SQL Editor to make processing more permissive during testing

-- 1) Use a larger time window (24h) and lower confidence threshold (0.3) when processing events
...
BEGIN
    ...
    LOOP
        -- Detect visits for this user over the last 24 hours (was 2 hours)
        FOR visit_record IN 
            SELECT * FROM detect_visits_from_location_events(user_record.user_id, 24)
            WHERE confidence >= 0.3  -- Lower threshold for testing (was 0.5)
        LOOP
...
-- 2) Reduce minimum dwell time from 10 to 5 minutes to make visit detection less strict
...
DECLARE
    min_dwell_minutes INTEGER := 5;  -- was 10
...
-- Optional: ensure cron job continues to run this updated function
