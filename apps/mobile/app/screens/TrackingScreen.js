...
export const TrackingScreen = () => {
    ...
    const [nativeQueue, setNativeQueue] = useState("");
    ...
    const getCurrentLocation = async () => {
        try {
            ...
            Alert.alert("Current Location", `Lat: ${position.coords.latitude.toFixed(6)}\nLng: ${position.coords.longitude.toFixed(6)}\nAccuracy: ${position.coords.accuracy?.toFixed(0)}m`);
        }
        catch (error) {
    ...
    return (<Screen preset="auto" contentContainerStyle={themed($screenContentContainer)} safeAreaEdges={["top"]}>
      <Text testID="tracking-heading" text="Location Tracking" preset="heading" style={themed($heading)}/>
      
      <Text text="Manage your location tracking settings" preset="subheading" style={themed($subheading)}/>

      {/* Tracking Control Card */}
      <Card style={themed($card)} heading="Tracking Status" ContentComponent={<View>
            <Switch value={isTracking} onPress={toggleTracking} label="Location Tracking" labelStyle={themed($toggleLabel)}/>

            <Text text={isTracking ? "Active" : "Stopped"} size="lg" weight="bold" style={[
                themed($statusText),
      ...
      <Card style={themed($card)} heading="Status Information" ContentComponent={<View>
            ...
            {nativeQueue ? (<Text text={nativeQueue} style={themed($infoText)}/>) : null}
            <Text text="Battery Optimization: SLC Mode" style={themed($infoText)}/>
          </View>}/>

      {/* Action Buttons */}
      <Button text="Get Current Location" style={themed($button)} onPress={getCurrentLocation}/>

      <Button text="Force Sync Events" style={themed($button)} preset="default" onPress={forceSync} disabled={queuedEvents === 0}/>

      {/* Help Text */}
      <Text text="When tracking is enabled, the app will automatically detect when you visit assigned accounts using Significant Location Changes (SLC) for optimal battery life." size="sm" style={themed($helpText)}/>
    </Screen>);
};
...
const $toggleLabel = ({ spacing }) => ({
    marginVertical: spacing.sm,
});
const $statusText = ({ spacing }) => ({
    marginTop: spacing.xs,
});
const $infoText = ({ spacing, colors }) => ({
...
//# sourceMappingURL=TrackingScreen.js.map